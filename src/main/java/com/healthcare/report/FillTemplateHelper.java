...
public class FillTemplateHelper extends PdfPageEventHelper {
	...
	public static ElementList parseHtml(String content, String style, TagProcessorFactory tagProcessors)
		...
		ClassPathResource resource = new ClassPathResource(style);
		CssFile cssFile = XMLWorkerHelper.getCSS(resource.getInputStream());
		...
		p.parse(new ByteArrayInputStream(content.getBytes(StandardCharsets.UTF_8.name())), Charset.forName("UTF-8"));
		return elements;
	}
}
