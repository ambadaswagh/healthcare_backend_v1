...
public class PlatFormAdminControllerTest {
	...
	String PLAT_FORM_ADMIN_API = "/api/platformAdmin";
	...
	public void testSaveAdmin() throws Exception {
		...
		this.mockMvc.perform(post(PLAT_FORM_ADMIN_API).contentType(MediaType.APPLICATION_JSON).content(jsonInString))
	...
	public void testGetAdmin() throws Exception {
		...
		this.mockMvc.perform(get(PLAT_FORM_ADMIN_API+"/1")).andExpect(status().isOk());
	}
	...
	public void testFindAllAdmin() throws Exception {
		...
		this.mockMvc.perform(get(PLAT_FORM_ADMIN_API)).andExpect(status().isOk());
	}
	...
	public void testUpdateAdmin() throws Exception {
		...
		ObjectMapper mapper = new ObjectMapper();
		String jsonInString = mapper.writeValueAsString(admin);
		this.mockMvc.perform(put(PLAT_FORM_ADMIN_API).contentType(MediaType.APPLICATION_JSON).content(jsonInString))
				.andExpect(status().isOk());
	}
	...
	public void testDisableAdmin() throws Exception {
		...
		this.mockMvc.perform(put(PLAT_FORM_ADMIN_API+"/1/disable")).andExpect(status().isOk());
	}
}
